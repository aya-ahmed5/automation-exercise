        ...
        if(propertyReader.getproperty("recordTest").equalsIgnoreCase("true")){
            try {
                ...
                if (propertyReader.getproperty("Excuation_Type").equalsIgnoreCase("local")) {
                    ...
                    // Start recording
                    recorder.get().start();
                    logs.infoMethod("Recording Started");
                }

            } catch(Exception e){
                logs.errorMethod("Failed to start recording: " + e.getMessage());
            }
    ...
    public static void stopRecording(String testMethodName) {
        try {
            if (recorder.get() != null) {
                ...
                logs.infoMethod("Video file saved at: " + videoFile.getAbsolutePath());


                // Convert the video to .mp4 format
                File mp4File = encodeRecording(videoFile);
                logs.infoMethod("Recording Stopped and Converted to MP4: " + mp4File.getName());
            }
        } catch (Exception e) {
            logs.errorMethod("Failed to stop recording: " + e.getMessage());
        } finally {
            recorder.remove();
        }
    }
    ...
    private static File encodeRecording(File sourceFile) {
        ...
        try {
            ...
            if (targetFile.exists()) {
                sourceFile.delete();
               logs.infoMethod("Deleted original AVI file: " + sourceFile.getName());
            }
        } catch (EncoderException e) {
           logs.errorMethod("Failed to convert video to MP4: " + e.getMessage());
        }
        return targetFile;
    }
}
